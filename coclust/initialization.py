...
def random_init_fuzzy_parameters(n_clusters, n_rows, n_cols):
    ...
    """
    U = np.random.dirichlet(np.ones(n_clusters),size=n_rows).astype(np.float128)
    V = np.random.dirichlet(np.ones(n_clusters),size=n_cols).astype(np.float128)
    return U, V
